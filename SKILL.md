...
CronCreate 的 prompt 必须包含以下结构（ID 通过 CronList() 查找）：
...
记录返回的 job_id。然后**立即在当前 turn 执行一次 cron prompt 中的任务**。
...
🔄 Loop 已结束
  ...
  [1] ✅ 保留修改 → 合并回原始分支，删除保护分支
  [2] ❌ 全部丢弃 → 切回原始分支，删除保护分支
```

## 关键规则

1. **启动前必须用户确认**
2. **cron 的 prompt 必须包含 CronDelete 自毁逻辑**
